...
# Using a stable, high-quality model
...
KEYWORDS = {
    ...
    'others': ['award', 'achievement', 'certified', 'winner', 'project', 'dean']
}
...
def generate_llm_comment(field_name, text, score):
    ...
    # Updated Prompt: Explicitly asking for length and detail
    prompt = (
    ...
    payload = {
        "inputs": prompt, 
        "parameters": {
            "max_new_tokens": 250,      # Increased from 150 to 250 words capacity
            "temperature": 0.7,
            "return_full_text": False,
            "wait_for_model": True      # Explicitly tell API to wait
        }
    }
    ...
    for attempt in range(5): # Increased from 3 to 5 attempts
        try:
            ...
            if isinstance(result, list) and 'generated_text' in result[0]:
                ...
                if 'loading' in err:
                    # Increased wait time from 10s to 20s for cold start
                    time.sleep(20) 
                    continue
                else:
                    # Log error but keep trying if possible
    ...
    # --- DYNAMIC FALLBACK (Only used if API is totally dead) ---
...
def process_candidate(candidate_id, pid=None):
    ...
    try:
        ...
        for field in fields:
            ...
            # RATE LIMITING PAUSE
            # Add a small delay between requests to avoid overwhelming the free API
            time.sleep(2) 
        ...
        vals = (scores['overall'], comments['overall'], scores['education'], comments['education'], scores['skills'], comments['skills'], scores['experience'], comments['experience'], scores['language'], comments['language'], scores['others'], comments['others'], confidence, candidate_id)
